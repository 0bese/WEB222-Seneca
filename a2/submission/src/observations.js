...
function speciesCoordinates(data) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    // Format the observation information and print it to the console
...
function speciesCoordinates2(data) {
  ...
  // Use forEach to iterate over each observation in the results array
  results.forEach((observation) => {
    const species = observation.species_guess;
    const location = observation.location.split(',');

    // Format the observation information as specified
    const formattedOutput = `"${species}" observed at coordinates (${location[0]},${location[1]})`;
...
function speciesCoordinates3(data) {
  ...
  // Use a for...of loop to iterate over each observation in the results array
  for (const observation of results) {
    const species = observation.species_guess;
    const location = observation.location.split(',');

    // Format the observation information as specified with a space after the comma
    const formattedOutput = `"${species}" observed at coordinates (${location[0]}, ${location[1]})`;
...
function observationsByQualityGrade(data, qualityGrade) {
  ...
  if (
    qualityGrade !== null &&
    qualityGrade.toLowerCase() !== 'research' &&
    qualityGrade.toLowerCase() !== 'casual' &&
    qualityGrade.toLowerCase() !== 'needs_id'
  ) {
  ...
  for (const observation of data.results) {
    if (
      ...
      (qualityGrade === null && observation.quality_grade === null)
    ) {
...
function observationsByQualityGrades(data, ...qualityGrades) {
  // TODO
  // Check if at least one qualityGrade is provided
  if (qualityGrades.length === 0) {
    throw new Error('At least one qualityGrade must be provided');
  }

  // Initialize an array to store the merged results
  let mergedResults = [];

  // Iterate through each qualityGrade and call observationsByQualityGrade
  for (const grade of qualityGrades) {
    // Use the spread operator (...) to pass the grade as a separate argument
    const gradeResults = observationsByQualityGrade(data, grade);

    // Merge the results with the mergedResults array
    mergedResults = [...mergedResults, ...gradeResults];
  }
...
function transformObservation(original) {
  // TODO
  const conservStat = original.conservation_status;

  // Access the status_name property from the conservation_status object
  const statName = conservStat ? conservStat.status_name : '';

  const obj = {
    id: original.id,
    name: original.species_guess.toLowerCase(),
    isExtinct: statName === 'extinct in the wild',
...
function transformObservations(data) {
  // TODO
  // Create a new empty array to hold all the transformed observations
  const transformedObservations = [];

  // Iterate over each observation object in the data results
  data.results.forEach((observation) => {
    // Transform the current observation and add it to the new array
    const transformedObservation = transformObservation(observation);
    transformedObservations.push(transformedObservation);
  });

  // Return the array containing all the transformed observations
...
function transformObservations2(data) {
  // TODO
  // Use the .map() method to create a new array of transformed observations
...
function getObservationsByTaxa(data, ...taxaNames) {
  // TODO
  // Initialize an array to store matching observations
  const matchingObservations = [];

  // Iterate over the provided taxa names
  taxaNames.forEach(function (taxonName) {
    // Filter observations based on the taxon name
    const observationsForTaxon = data.results.filter(function (observation) {
      return observation.taxon.iconic_taxon_name === taxonName;
    });

    // If there are matching observations, add them to the result
    if (observationsForTaxon.length > 0) {
      matchingObservations.push(...observationsForTaxon);
    }
  });
...
function getObservationsByLocation(data, options = {}) {
  // TODO
  // If no location options are provided, return all observations
  if (!options.lat && !options.lng) {
    return data.results;
  }

  return data.results.filter((observation) => {
    const observationLocation = observation.location;
    const [observationLat, observationLng] = observationLocation.split(',');

    if (options.lat) {
      const latOptions = options.lat;
      if (typeof latOptions === 'object') {
        ...
        const latValue = parseFloat(latOptions);
        if (parseFloat(observationLat) !== latValue) {
          return false;
        }
      }
    }
    ...
    if (options.lng) {
      const lngOptions = options.lng;
      if (typeof lngOptions === 'object') {
        ...
        const lngValue = parseFloat(lngOptions);
        if (parseFloat(observationLng) !== lngValue) {
          return false;
        }
      }
    }
...
function getPlaceURLs(data) {
  // TODO
  // Check if data is empty or doesn't contain results
  ...
  for (const observation of data.results) {
    // Check if place_ids are present for the observation
    if (observation.place_ids && observation.place_ids.length > 0) {
      // Generate a URL for each place_id and add it to the placeURLs array
      for (const placeId of observation.place_ids) {
...
function getSpeciesObservations(data) {
  ...
  // Create an object to store species observations counts
  ...
  for (const observation of data.results) {
    if (observation.taxon && observation.taxon.name) {
      ...
      // Increment the count for the species or initialize it to 1
      speciesObservations[speciesName] = (speciesObservations[speciesName] || 0) + 1;
    }
  }
...
function extractSpeciesNames(data) {
  ...
  // Loop through all results in the data
  for (const item of data.results) {
    // Check if the species_guess property exists and is not null or undefined
    if (item.species_guess != null) {
      // Check if the species name is not already in the uniqueSpeciesNames array
      if (!uniqueSpeciesNames.includes(item.species_guess)) {
        // Add the unique species name to the array
        uniqueSpeciesNames.push(item.species_guess);
      }
    }
  }
...
function extractSpeciesNames2(data) {
  ...
  for (const item of data.results) {
    if (item.species_guess) {
      uniqueSpeciesNames.add(item.species_guess);
    }
  }

  return Array.from(uniqueSpeciesNames);
}
