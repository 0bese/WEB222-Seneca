...
function transformObservation(original) {
  // TODO
  //let extinctCond = 'extinct in the wild'
  obj = {
    id: original.id,
    name: original.species_guess.toLowerCase(),
    isExtinct: original.status_name === 'extinct in the wild' ? true : false,
...
function getObservationsByLocation(data, options = {}) {
  ...
  return data.results.filter((observation) => {
    const observationLat = observation.latitude;
    const observationLng = observation.longitude;
    ...
    if (options.lat) {
      ...
      if (typeof latOptions === 'object') {
        ...
        if (typeof min === 'number' && typeof max === 'number') {
          const latThreshold = 0.000001;
          if (max - min < latThreshold) {
            return false; // Too close together, skip this observation
          }
          if (observationLat < min || observationLat > max) {
            return false;
          }
        }
      } else if (typeof latOptions === 'number') {
        if (observationLat !== latOptions) {
          return false;
        }
      }
    }
    ...
    if (options.lng) {
      ...
      if (typeof lngOptions === 'object') {
        ...
        if (typeof min === 'number' && typeof max === 'number') {
          const lngThreshold = 0.000001; // Adjust the threshold as needed
          if (max - min < lngThreshold) {
            return false; // Too close together, skip this observation
          }
          if (observationLng < min || observationLng > max) {
            return false;
          }
        }
      } else if (typeof lngOptions === 'number') {
        if (observationLng !== lngOptions) {
          return false;
        }
      }
    }
