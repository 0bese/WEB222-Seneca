...
function toCamelCase(name, uppercase) {
  ...
  // Capitalize the first letter of each part except the first one if not uppercase
...
function createLinkTag(rel, href) {
  // Replace this comment with your code...
  var newRel = rel.trim();
  var newHref = href.trim();

  if (newRel.includes(' ') && newHref.includes(' ')) {
    return '<link rel="' + newRel + '" href="' + newHref + '">';
  } else if (newRel.includes('\t') && newHref.includes('\t')) {
    return '<link rel="' + newRel + '" href="' + newHref + '">';
  } else if (newRel.includes('\n') && newHref.includes('\n')) {
    return '<link rel="' + newRel + '" href="' + newHref + '">';
  }
...
function parseDateString(value) {
  ...
  if (parts.length !== 3) {
    throw new Error('invalid date format, expected....');
  }
...
function toDateString(value, format) {
  ...
  // Check if the format is one of the three allowed formats
  const allowedFormat = ['YYYY/MM/DD', 'DD/MM/YYYY', 'MM/DD/YYYY'];
  ...
  // Format the date string based on the format parameter
  switch (format) {
    case 'YYYY/MM/DD':
...
function normalizeTime(value) {
  ...
  // Regular expression to match both time formats
...
function formatTimes() {
  ...
  for (let i = 0; i < arguments.length; i++) {
    const normalizedTime = normalizeTime(arguments[i]);

    // Check if the time was successfully normalized
    if (normalizedTime !== null) {
      validTimes.push(normalizedTime);
    }
  }

  // Format the valid times as a string enclosed in square brackets
  const formattedTimes = `[${validTimes.join(', ')}]`;
...
function mimeTypeFromFilename(filename) {
  ...
  if (extension.length === 1) {
    // Handle unknown extensions or files with no extension
    ...
    // Use a switch statement to map extensions to MIME types
    switch (extension[1]) {
      case 'txt':
        return 'text/plain';
      case 'html':
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  ...
  if (extraData) {
    // Encode and append extraData if provided
    const encodedExtraData = encodeURIComponent(extraData);
    endpoint += `/${encodedExtraData}`;
  }
